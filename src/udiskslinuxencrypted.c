...
/* runs in thread dedicated to handling method call */
static gboolean
handle_header_restore (UDisksEncrypted       *encrypted,
                      GDBusMethodInvocation *invocation,
                      const gchar           *backup_file,
                      GVariant              *options)
{
    ...
    udisks_encrypted_complete_header_restore (encrypted, invocation);
    udisks_simple_job_complete (UDISKS_SIMPLE_JOB (job), TRUE, NULL);
...
encrypted_iface_init (UDisksEncryptedIface *iface)
{
  ...
  iface->handle_header_restore      = handle_header_restore;
